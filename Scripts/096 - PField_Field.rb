...
class LocationWindow
  ...
  def initialize(name)
    ...
    @sprites["overlay"].bitmap=Bitmap.new(20*name.length,Graphics.height*4)
    @sprites["overlay"].z=9999999
		@sprites["overlay"].bitmap.font = Font.new("SAO UI",36)
    @sprites["overlay"].bitmap.font.color = Color.new(65,65,65)
    ...
    @shadowColor=Color.new(148,148,165)
		...
		@dim = name.length
		@index = pbGetSymbolIndex(name)
    @sprites["End"] = Sprite.new
    @sprites["End"].bitmap = BitmapCache.load_bitmap("Graphics/Pictures/route-bar-end")
    @sprites["End"].x = 0 - @sprites["End"].bitmap.width
    @sprites["End"].y = 0	
		@sprites["Area"] =[]
    for i in 0..@dim
			@sprites["Area"][i] = Sprite.new()
			@sprites["Area"][i].bitmap = BitmapCache.load_bitmap("Graphics/Pictures/route-bar-bg")
			@sprites["Area"][i].x = 0 - (@sprites["Area"][0].bitmap.width * (i+1)) - @sprites["End"].bitmap.width
			@sprites["Area"][i].y = 0
		end
		@sprites["Alpha"] = Sprite.new
    @sprites["Alpha"].bitmap = BitmapCache.load_bitmap("Graphics/Pictures/alpha-bg")
    @sprites["Alpha"].x = 0 - @sprites["Alpha"].bitmap.width - (@sprites["Area"][0].bitmap.width * @dim) - @sprites["End"].bitmap.width
    @sprites["Alpha"].y = 0
		@sprites["Logo"] = Sprite.new
    @sprites["Logo"].bitmap = BitmapCache.load_bitmap("Graphics/Pictures/Signpost_Logos/"+ @index.to_s )
    @sprites["Logo"].x = 0 - @sprites["Alpha"].bitmap.width - (@sprites["Area"][0].bitmap.width * @dim) - @sprites["End"].bitmap.width+12
    @sprites["Logo"].y = 7
  ...
  def dispose
    ...
    @sprites["Alpha"].dispose
		@sprites["Logo"].dispose
		for i in 0..@dim
			@sprites["Area"][i].dispose
		end
		@sprites["End"].dispose
  end
  ...
  def update
    ...
    if @frames>80
      @sprites["Alpha"].x -= MOV
			@sprites["Logo"].x -= MOV if !@sprites["Logo"].disposed?
			for i in 0..@dim
				@sprites["Area"][i].x -=MOV if !@sprites["Area"][i].disposed?
			end
			@sprites["End"].x -= MOV
      @overlay.clear
      textPositions=[]
      textPositions.push([_INTL("{1}", $game_map.name),(@sprites["Alpha"].x)+50,5,0,@baseColor,@shadowColor])
      pbDrawTextPositions(@overlay,textPositions)
      @overlay.dispose if @sprites["End"].x+@sprites["End"].bitmap.width<0
      @window.dispose if @sprites["End"].x+@sprites["End"].bitmap.width<0
      @sprites["Alpha"].dispose if @sprites["Alpha"].x+@sprites["Alpha"].bitmap.width+(@sprites["Area"][0].bitmap.width * @dim)+@sprites["End"].bitmap.width<0
			@sprites["Logo"].dispose if @sprites["Alpha"].x+@sprites["Alpha"].bitmap.width+(@sprites["Area"][0].bitmap.width * @dim)+@sprites["End"].bitmap.width - 10 <0
			for i in 0..@dim
				@sprites["Area"][@dim-i].dispose if @sprites["Area"][@dim-i].x+(@sprites["Area"][0].bitmap.width * @dim)+@sprites["End"].bitmap.width<0
			end
			@sprites["End"].dispose if @sprites["End"].x+@sprites["End"].bitmap.width<0
    else
      @sprites["Alpha"].x+= MOV if @sprites["Alpha"].x<0
			@sprites["Logo"].x+= MOV if @sprites["Alpha"].x<0
			for i in 0..@dim
				@sprites["Area"][i].x +=MOV if @sprites["Area"][i].x<@sprites["Alpha"].bitmap.width + (@sprites["Area"][0].bitmap.width * (@dim-i-1)) 
			end
			@sprites["End"].x+= MOV if @sprites["End"].x<(@sprites["Alpha"].bitmap.width + (@sprites["Area"][0].bitmap.width * @dim))
      @overlay.clear
      textPositions=[]
      textPositions.push([_INTL("{1}", $game_map.name),(@sprites["Alpha"].x)+50,5,0,@baseColor,@shadowColor])
      pbDrawTextPositions(@overlay,textPositions)
