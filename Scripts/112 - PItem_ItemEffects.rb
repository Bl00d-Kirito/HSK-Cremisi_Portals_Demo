...
def pbRepel(item,steps)
  if $PokemonGlobal.repel>0
    ...
    @window = Sprite.new
		@window.bitmap = Bitmap.new("Graphics/Pictures/RepelWindow")
		@window.x = (Graphics.width-@window.bitmap.width)/2
		@window.y = -(@window.bitmap.height)
		@imageText = Sprite.new
    @imageText.bitmap = Bitmap.new(DEFAULTSCREENHEIGHT,DEFAULTSCREENWIDTH)
    @imageText.bitmap.draw_text(0,7, DEFAULTSCREENWIDTH, 24,$PokemonGlobal.repel.to_s,1)
		@imageText.y = -(@window.bitmap.height)
		for i in -(@window.bitmap.height)..0
			Graphics.update
			@window.y += 1
			@imageText.y += 1
		end
...
Events.onStepTaken+=proc {
   if $game_player.terrain_tag!=PBTerrain::Ice   # Shouldn't count down if on ice
		if $PokemonGlobal.repel>0
			...
			@imageText = Sprite.new
      @imageText.bitmap = Bitmap.new(DEFAULTSCREENHEIGHT,DEFAULTSCREENWIDTH)
      @imageText.bitmap.draw_text(0,7, DEFAULTSCREENWIDTH, 24,($PokemonGlobal.repel-1).to_s,1)
