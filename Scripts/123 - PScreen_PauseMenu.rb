...
class PokemonMenu
  ...
  def pbStartPokemonMenu
    ...
    commands[cmdBag=commands.length]=_INTL("Bag") if !pbInBugContest? && $game_variables[31]>0
    commands[cmdPokegear=commands.length]=_INTL("Pokégear") if $Trainer.pokegear
    commands[cmdTrainer=commands.length]=$Trainer.name if $game_variables[32]>0
    if pbInSafari?
      if SAFARISTEPS<=0
