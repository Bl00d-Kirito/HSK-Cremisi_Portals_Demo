...
class CommandMenuDisplay
  ...
  def initialize(viewport=nil)
    ...
    @msgbox=Window_UnformattedTextPokemon.newWithSize(
       "",60,Graphics.height-96+2,176,96,viewport)
...
class CommandMenuButtons < BitmapSprite
  def initialize(index=0,mode=0,viewport=nil)
    super(319,96,viewport)
    self.x=Graphics.width-319
  ...
  def refresh(index,mode=0)
    ...
    for i in 0...4
      ...
      #y=((i/2)==0) ? 6 : 48
			y=0
			x=80*i
      self.bitmap.blt(x,y,@buttonbitmap.bitmap,Rect.new(0,cmdarray[i]*96,80,96))
    end
    for i in 0...4
			next if i!=index
      #x=((i%2)==0) ? 0 : 126
      #y=((i/2)==0) ? 6 : 48
			y=0
			x=80*i
      self.bitmap.blt(x,y,@buttonbitmap.bitmap,Rect.new(80,cmdarray[i]*96,80,96))
    end
  end
end
...
class FightMenuButtons < BitmapSprite
  ...
  def initialize(index=0,moves=nil,viewport=nil)
    ...
    @megaevobitmap=AnimatedBitmap.new(_INTL("Graphics/Pictures/battleMegaEvo"))
		@arrow=AnimatedBitmap.new(_INTL("Graphics/Pictures/selectArrow"))
  ...
  def refresh(index,moves,megaButton)
    ...
    for i in 0...4
      ...
      y=((i/2)==0) ? 0 : 48   #6 : 48
      y+=UPPERGAP
      self.bitmap.blt(x,y,@buttonbitmap.bitmap,Rect.new(0,moves[i].type*48,250,48))
      textpos.push([_INTL("{1}",moves[i].name),x+35,y+8,0,
    ...
    for i in 0...4
      ...
      y=((i/2)==0) ? 0 : 48
      y+=UPPERGAP
      self.bitmap.blt(x,y,@buttonbitmap.bitmap,Rect.new(250,moves[i].type*48,250,48))
      self.bitmap.blt(529,20+UPPERGAP,@typebitmap.bitmap,Rect.new(0,moves[i].type*28,64,28))
			self.bitmap.blt(616,20+UPPERGAP,@categorybitmap.bitmap,Rect.new(0,moves[i].category*28,64,28))
			self.bitmap.blt(x+35,y+14,@arrow.bitmap,Rect.new(0,0,20,20))
      textpos.push([_INTL("{1}",moves[i].name),x+60,y+8,0,
         PokeBattle_SceneConstants::MENUBASECOLOR,PokeBattle_SceneConstants::MENUSHADOWCOLOR])
      if moves[i].totalpp>0
        ppfraction=(4.0*moves[i].pp/moves[i].totalpp).ceil
				if moves[i].basedamage==1
					damage="---"
				else
					damage=moves[i].basedamage
				end
        textpos.push([_INTL("PP: {1}/{2} | {3}",moves[i].pp,moves[i].totalpp,damage),
           603,50+UPPERGAP,2,ppcolors[(4-ppfraction)*2],ppcolors[(4-ppfraction)*2+1]])
      end
    end
    pbDrawTextPositions(self.bitmap,textpos)
    if megaButton>0
      self.bitmap.blt(225,69,@megaevobitmap.bitmap,Rect.new(0,(megaButton-1)*50,50,50))
    end
  end
end
...
=begin
		enemySpace = 0
    if @battler.index == 1 || @battler.index == 3
      enemySpace = 40
    end
=end
    ...
    # fill with white (shows what the HP used to be)
    ...
    hpGaugeY=PokeBattle_SceneConstants::HPGAUGE_Y
      ...
      #   @starthp*hpGaugeSize/@battler.totalhp,19,Color.new(0,0,0))
      self.bitmap.fill_rect(@spritebaseX+hpGaugeX,hpGaugeY,
         PokeBattle_SceneConstants::HPGAUGESIZE,17,Color.new(200,200,200))
    #end
    # fill with HP color
    self.bitmap.fill_rect(@spritebaseX+hpGaugeX,hpGaugeY,hpgauge,2,hpcolors[hpzone*2])
    self.bitmap.fill_rect(@spritebaseX+hpGaugeX,hpGaugeY+2,hpgauge,15,hpcolors[hpzone*2+1])
    if @showexp
      #Calcolo variabili
      expGaugeX=PokeBattle_SceneConstants::EXPGAUGE_X
      expGaugeY=PokeBattle_SceneConstants::EXPGAUGE_Y
      # fill with white (shows what the EXP used to be)
      self.bitmap.fill_rect(@spritebaseX+expGaugeX,expGaugeY,PokeBattle_SceneConstants::EXPGAUGESIZE,5,Color.new(255,255,255))
      # fill with EXP color
      self.bitmap.fill_rect(@spritebaseX+expGaugeX,expGaugeY,self.exp,2,
         PokeBattle_SceneConstants::EXPCOLORSHADOW)
      self.bitmap.fill_rect(@spritebaseX+expGaugeX,expGaugeY+2,self.exp,3,
    ...
    pbSetSystemFont(self.bitmap)
    textpos=[
       [@battler.name,@spritebaseX+12,3,false,base,shadow]
    ]
    #genderX=self.bitmap.text_size(pokename).width
    #genderX+=@spritebaseX-30
		imagepos=[]
    if @battler.gender==0 # Male
      imagepos=[["Graphics/Pictures/male.png",@spritebaseX+-15,8,0,0,-1,-1]]
    elsif @battler.gender==1 # Female
      imagepos=[["Graphics/Pictures/female.png",@spritebaseX-15,8,0,0,-1,-1]]
    end
		pbDrawImagePositions(self.bitmap,imagepos)
    pbDrawTextPositions(self.bitmap,textpos,["SAO UI","Verdana"],30)
    pbSetSmallFont(self.bitmap)
    textpos=[
       [_INTL(@battler.level.to_s()),@spritebaseX+194,5,true,base,shadow]
    ]
		pbDrawTextPositions(self.bitmap,textpos,["SAO UI","Verdana"],30)
    if (@battler.index==0 || @battler.index==2)
      hpstring=_ISPRINTF("{1: 2d}/{2: 2d}",self.hp,@battler.totalhp)
			textpos=[[hpstring,@spritebaseX+268,32,2,base,shadow]]
		else
			hpstring=_ISPRINTF("{1: 2d}%",self.hp*100/@battler.totalhp)
			textpos=[[hpstring,@spritebaseX+255,32,2,base,shadow]]
    end
		pbDrawTextPositions(self.bitmap,textpos,["SAO UI","Verdana"],24)
=begin
    hpstring=_ISPRINTF("{1: 2d}/{2: 2d}",self.hp,@battler.totalhp)
    hpnum = Sprite.new
    hpnum.x = @spritebaseX+330
    hpnum.y = 32
    hpnum.bitmap = Bitmap.new(self.bitmap.width,self.bitmap.height)
    hpnum.bitmap.font.name = ["SAO UI","Verdana"]
    hpnum.bitmap.font.size = 22
    hpnum.bitmap.draw_text(0,0,100,30,hpstring)
=end
    ...
    if @battler.pokemon.isShiny?
      ...
      imagepos.push(["Graphics/Pictures/shiny.png",@spritebaseX+130,14,0,0,-1,-1])
    end
		if @battler.status>0
      self.bitmap.blt(@spritebaseX+213,0,@statuses.bitmap,
				Rect.new(0,(@battler.status-1)*36,52,36))
			@blockPos+=1
    end
    if @battler.isMega?
      imagepos.push(["Graphics/Pictures/battleMegaEvoBox.png",@spritebaseX+213+(@blockPos*37),0,0,0,-1,-1])
			@blockPos+=1
    end
    if @battler.owned && (@battler.index&1)==1
      imagepos.push(["Graphics/Pictures/battleBoxOwned.png",@spritebaseX+213+(@blockPos*37),0,0,0,-1,-1])
			@blockPos+=1
    end
    pbDrawImagePositions(self.bitmap,imagepos)
    #Aggiunta qui
...
# Return values: 0=Fight, 1=Bag, 2=Pokémon, 3=Run, 4=Call
  ...
  def pbCommandMenuEx(index,texts,mode=0)      # Mode: 0 - regular battle
    ...
    loop do
      ...
      if Input.trigger?(Input::LEFT) && cw.index > 0 #(cw.index&1)==1
        pbPlayCursorSE()
        cw.index-=1
      elsif Input.trigger?(Input::RIGHT) && cw.index < 3 #(cw.index&1)==0
        pbPlayCursorSE()
        cw.index+=1
=begin
